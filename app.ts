...
let activeProjectCategory: ProjectCategory | "All" = "All";
...
function renderProjects() {
    ...
    const items = isHomepage()
        ...
        : projects.filter(project => activeProjectCategory === "All" || project.categories?.includes(activeProjectCategory));
...
function setupProjectFilters() {
    ...
    filters.onclick = (event) => {
        const button = (event.target as HTMLElement).closest<HTMLButtonElement>('[data-project-filter]');
        if (!button) return;
        activeProjectCategory = button.dataset.projectFilter as ProjectCategory | "All";
        setupProjectFilters();
        renderProjects();
    };
}
...
function setupExperienceCards() {
    ...
    const revealCard = (card: Element) => card.classList.add('is-visible');
    if (!('IntersectionObserver' in window)) {
